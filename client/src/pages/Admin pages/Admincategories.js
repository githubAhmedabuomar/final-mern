  ...
  const handleupdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://mern-e-commerce-abuelmaati-e4b4bf37fe38.herokuapp/api/v1/category/updateCategory/${id}`,
        { newname },
        {
  ...
  const handledelete = async (name) => {
    try {
      const { data } = await axios.delete(
        `https://mern-e-commerce-abuelmaati-e4b4bf37fe38.herokuapp/api/v1/category/deleteCategory/${name}`
      );
      window.location.reload();
    } catch (error) {
      console.log(error);
